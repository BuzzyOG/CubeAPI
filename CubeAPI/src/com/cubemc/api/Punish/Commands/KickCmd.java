 ...
 * 24/05/2015
 */
public class KickCmd extends CommandBase<PunishManager> {

    public KickCmd(PunishManager plugin) {
        super(plugin, Rank.MODERATOR, Arrays.asList("kick"));
        CMDh.register("kick", "<player> <reason>", Rank.MODERATOR, "Kicks a player");
    }

    @Override
    public void execute(Player p, String[] args) {
        if (args==null || args.length<2){
            p.sendMessage(M.usage("kick", "<player> <reason>"));
            return;
        }
        ...
        target.kickPlayer("§cYou have been kicked from CubeMC.\n\n" +
                "§cReason - §7" + reason + "\n" +
                "§cStaff Member - §7" + p.getName());

        M.staff(p.getDisplayName() + " §7kicked " + tdn + " §7with reason: §c" + reason + "§7.");
